...
# load input
input_file_name = input_directory_name + "L.%s_filtered_fRNAdb_input_for_NNSE_robustness_estimation.txt" % (L)
...
for index,input_ID in enumerate(input_IDs):
    ...
    # execute program 'get_size_NN_w_RMU'
    NNSE_output_file_name = results_directory_name + "NNSE_output/" + "NNSE_output_%s.txt" % (input_IDs[index])
    subprocess.call("./get_size_NN_w_RMU -s '%s' -m %s > '%s'" % (input_pred_structures[index],number_measurements,NNSE_output_file_name),shell=True)
    
    # read output file (three lines below would need to be updated if number_measurements!=1)
    NNSE_NN_rob_est = float(np.loadtxt(NNSE_output_file_name, usecols=(13,), skiprows=6+L+1, unpack=True))
    ...
    results_NNSE_NN_rob_est_list.append(NNSE_NN_rob_est)
...
file_name = results_directory_name + "L.%s_fRNAdb_NNSE_NN_rob_estimates_section.%s.txt" % (L,input_list_section_number)
f = open(file_name,"w")
